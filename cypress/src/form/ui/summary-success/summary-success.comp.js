export const getViewProposalButton = () => cy.get('label="Frequency"');

export const getEmailProposalButton = () => cy.get('label="Days Serviced"');

export const getCopyLinkButton = () => cy.get('label="Time Window"');

export const getSuccessMessage = () => cy.get('div[id="root"]')
  .contains('Your Proposal was saved successfully');
...
export const getDocumentIdFromSummaryPage = () => cy.location('href').then((url) => {
  const documentId = url.toString().split('/')[url.toString().split('/').length - 2];
  return documentId;
});
